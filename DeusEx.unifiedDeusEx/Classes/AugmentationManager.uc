...
simulated function int NumAugsActive(optional bool onlyActive)
{
	local Augmentation anAug;
	local int count;
...
function DeactivateAll(optional bool force)
{
	local Augmentation anAug;
	...
	while(anAug != None)
	{
		if (anAug.bIsActive && (anAug.augType == active || force)) //Justice: Only deactivate active type augs
		{
			anAug.Deactivate();
...
//Justice: Since my augs handle all of this in their own way, I'm replacing this with the default
simulated function Float CalcEnergyUse(float deltaTime)
{
	local float energyUse, energyMult;
	local Augmentation anAug;
...
{	
	...
	{
		//Justice: Don't let the player activate non passive/automatic augs if they have no energy
		if (Player.Energy <= 0 && anAug.augType == active)
		{
			Player.ClientMessage(Player.EnergyDepleted);
			Player.PlaySound(FirstAug.DeactivateSound, SLOT_None);
			return False;
		}
