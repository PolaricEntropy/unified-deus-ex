...
// UI scaling factor: supported values are 1 (default) or 2, and appropriate DeusExUI.u (with correspondingly-sized assets) must be loaded
var globalconfig int dxEnhancedGUIScaleMultiplier; // controls the UI dimension-scaling factor

// native Functions
// !!! kocmo: adding new fields must occur right before native func declarations
...
function MaintainEnergy(float deltaTime)
{
	...
	if ((!IsInState('Dying')) && (!IsInState('Paralyzed')))
   {
	  ...
	  //Justice: Special case for when energy is at 0 and we're regenerating with an aug
	  if(energy == 0)
	  {
		energyUse = AugmentationSystem.CalcEnergyUse(deltaTime);
		if(energyUse < 0)
...
exec function ActivateAugmentation(int num)
{
	...
	/*
	Justice: I only want this to happen to non passive/automatic augs, so I've moved it to ActivateAugsByKey
	if (Energy == 0)
	{
		ClientMessage(EnergyDepleted);
...
simulated function PlayFootStep()
{
	...
	{
		//Justice: Athletics makes you faster, but not louder
		skillReduction = 1.0;
		if(!bIsWalking)
		{
			skillReduction = SkillSystem.getSkillLevel(class'SkillSwimming');
	
			switch(skillReduction)
			{
				case 1.0:
					skillReduction = 1.0;
					break;
		...
		speedFactor = (VSize(Velocity) / skillReduction)/180.0;
	}
...
function HighlightCenterObject()
{
	...
	if (FrobTime >= 0.1)
	{
		if(bSpyDroneActive) //Justice: Let the spy drone frob things
		{
			StartTrace = aDrone.Location;
		...
		if(bSpyDroneActive) //Justice: Determine what the drone can frob based on the augmentation level
		{
	...
	if (Velocity.Z < -1.4 * JumpZ)
	{
		...
		if ((Velocity.Z < -700) && (ReducedDamageType != 'All'))
			if ( Role == ROLE_Authority )
            {
				...
				if(dmg > 0) //Justice: If you don't take any damage, don't play the pain sound
		...
		MakeNoise(0.1 * Level.Game.Difficulty * runSilentValue); //Justice: Reduce volume based on run silent					
	bJustLanded = true;
}
...
function DoJump( optional float F )
{
	...
	{
		if ( Role == ROLE_Authority )
			PlaySound(JumpSound, SLOT_None, 1.5 * runSilentValue, true, 1200, (1.0 - 0.2*FRand()) * pitch ); //Justice: Reduce volume based on run silent
		if ( (Level.Game != None) && (Level.Game.Difficulty > 0) )
			MakeNoise(0.1 * Level.Game.Difficulty * runSilentValue); //Justice: For the AI as well
		PlayInAir();
		...
		//Justice: Athleticism affects jump height
		// kocmo: ... but we still have default jump height at UNTRAINED, to avoid getting stuck on the roof of the warehouse
		augLevel = SkillSystem.getSkillLevel(class'SkillSwimming');
...
function float GetCurrentGroundSpeed()
{
   ...
   augValue = AugmentationSystem.GetAugLevelValue(class'AugSpeed');
	...
	//Justice: Swimming is now athleticism, which affects run speed
...
function CreateDrone()
{
	...
	if (aDrone != None)
	{
		...
		aDrone.Speed = 5 * spyDroneLevelValue; //Justice: Let's up this to a more reasonable level
...
function DroneExplode()
{
	...
	local float powerReduce;

	if(AugmentationSystem.GetClassLevel(class'AugPower') >= 0.0)
		powerReduce = AugmentationSystem.getAugLevelValue(class'AugPower');
	else
		powerReduce = 1.0;
	
	if (aDrone != None && energy >= 30 * powerReduce)
	{
		aDrone.Explode(aDrone.Location, vect(0,0,1));
		
		//Justice: Exploding now costs energy
		EnergyDrain += 30 * powerReduce;
		EnergyDrainTotal += 30 * powerReduce;
	...
	else if(energy <= 30 * powerReduce)
		clientMessage("Bio-electric energy reserves insufficient");
}
...
function bool HandleItemPickup(Actor FrobTarget, optional bool bSearchOnly)
{
	...
	else 
	{
		...
		if (foundItem != None)
		{
			...
			{ //Justice: A pain in the ass for them, but a blessing for me
				if (DeusExWeapon(foundItem).AmmoType.AmmoAmount >= DeusExWeapon(foundItem).AmmoType.MaxAmmo + 2 * SkillSystem.GetSkillLevel(class'SkillDemolition'))
				{
					ClientMessage(TooMuchAmmo);
					bCanPickup = False;
				}
...
function UpdateAmmoBeltText(Ammo ammo)
{
	...
	//Justice: I don't know why this was deleted, but I'm putting it back so ammo text will be updated correctly
	inv = Inventory;
	while(inv != None)
	{
				...
				if (POVCorpse(item).carcClassString != "")
				{
					...
					if (carcClass != None)
					{
						...
						if (carc != None)
						{
							...
							//Justice: Save the new unconscious vars too
...
function TakeDamage(int Damage, Pawn instigatedBy, Vector hitlocation, Vector momentum, name damageType)
{
	...
	local AugHealing regen;
	
	if ( bNintendoImmunity )
		return;

	if(AugmentationSystem != None) //Justice: notify regen that it should take a break
	{
		regen = AugHealing(AugmentationSystem.findAugmentation(class'AugHealing'));
		if(AugmentationSystem.GetClassLevel(class'AugHealing') >= 0.0)
			regen.haloTimer = AugmentationSystem.GetAugLevelValue(class'AugHealing');
	}
   ...
   if (Level.NetMode != NM_Standalone)
	...
	//Justice: Kicks you off of your computer if you get shot, set on fire, etc.
	if(activeComputer != None && (damageType == 'Shot' || damageType == 'AutoShot' || damageType == 'Shell' || damageType == 'Exploded' || damageType == 'Flamed'))
		activeComputer.termwindow.CloseScreen("EXIT");
}
...
function bool DXReduceDamage(int Damage, name damageType, vector hitLocation, out int adjustedDamage, bool bCheckOnly)
{
		...
		(damageType == 'HalonGas')  || (damageType == 'PoisonEffect') || (damageType == 'Poison'))
	{
		...
		{
			newDamage *= augLevel;
			EnergyDrain += damage * 0.4 * powerReduce;
			EnergyDrainTotal += damage * 0.4 * powerReduce;
		}
...
//			if ((suit.Owner == Self) && suit.bActive)
     /* if (UsingChargedPickup(class'HazMatSuit'))
			...
			//Justice: Code for handling my new improved hazmat suit
		if ((damageType == 'TearGas') || (damageType == 'PoisonGas') || (damageType == 'Radiation') ||
			(damageType == 'HalonGas')  || (damageType == 'PoisonEffect') || (damageType == 'Poison') ||
			(damageType == 'Flamed') || (damageType == 'EMP') || (damageType == 'Shocked')) //Damage types that it was supposed to protect you from, but didn't
		{
			for (ChargedItem = Inventory; (ChargedItem != None); ChargedItem = ChargedItem.inventory)
				if ((ChargedItem.class == class'HazMatSuit') && (ChargedItem.bActive))
					suit = HazMatSuit(ChargedItem);
					
			if(suit != None)
			{
				skillLevel = SkillSystem.GetSkillLevelValue(class'SkillEnviro');
				newDamage *= 0.75 * skillLevel;
				suit.charge -= damage * skillLevel;
			}
		}
	...
	if ((damageType == 'Shot') || (damageType == 'Sabot') || (damageType == 'Exploded') || (damageType == 'AutoShot') || (damageType == 'Shell'))
	{
		...
		//Justice: ...and for armor
		for (ChargedItem = Inventory; (ChargedItem != None); ChargedItem = ChargedItem.inventory)
				if ((ChargedItem.class == class'BallisticArmor') && (ChargedItem.bActive))
					armor = BallisticArmor(ChargedItem);
					
		if(armor != None)
		{
			skillLevel = SkillSystem.GetSkillLevelValue(class'SkillEnviro');
			newDamage *= 0.75 * skillLevel;
			armor.charge -= damage * 2 * skillLevel;
		}
	}
	...
	if ((damageType == 'Shot') || (damageType == 'AutoShot') || (damageType == 'Shell') || (damageType == 'ExplodeShot'))
	{
		...
		{
			newDamage *= augLevel;
			EnergyDrain += damage * 0.5 * powerReduce;
			EnergyDrainTotal += damage * 0.5 * powerReduce;
		}
	}
	...
	{
		if (AugmentationSystem != None)
			if(AugmentationSystem.GetClassLevel(class'AugEMP') >= 0.0)
				augLevel = AugmentationSystem.GetAugLevelValue(class'AugEMP');
			else
				augLevel = -1; //Justice: Prevent the aug from taking effect when it isn't on
...
// ----------------------------------------------------------------------
// TentativeSkillPointsAdd_ShifterNewSkillSystem()
// ----------------------------------------------------------------------

function TentativeSkillPointsAdd_ShifterNewSkillSystem(int numPoints) { // Conditionally add skill awards from Shifter skill system only if bNewSkill system is set to true in DeusEx.ini
	if(DeusExGameInfo(Level.Game) != None && DeusExGameInfo(Level.Game).bNewSkillSystem)
		SkillPointsAdd(numPoints);
}
