...
state Activated
{
	...
	function BeginState()
	{
		...
		if (player != None)
		{
			skill = player.skillSystem.getSkillLevel(class'SkillMedicine');
			
			player.ClientMessage(Sprintf(msgRecharged, rechargeAmount[skill]));
	
			player.PlaySound(sound'BioElectricHiss', SLOT_None,,, 256);

			player.Energy += rechargeAmount[skill];
...
function bool UpdateInfo(Object winObject)
{
	...
	winInfo.AppendText(Sprintf(RechargesLabel, RechargeAmount[DeusExPlayer(Owner).skillSystem.getSkillLevel(class'SkillMedicine')]));
