...
var float lastDamageTime; //Justice: For fire propagation
			...
			foreach CycleActors(Class'Pawn', candidate, CycleIndex)
			{
				...
				if (bValidEnemy || bPotentialEnemy)
				{
					...
					if ((bPlayer && bCheckPlayer) || (!bPlayer && bCheckOther))
					{
						...
						} // Justice: Skill system disabled by default for BioMod
					}
					if (count >= 1)
						break;
				}
...
function bool GenerateRandomInventory()
{
	...
	local bool bShifterEnableRandomInventory, bShifterEnableWeaponModRateOfFire, bHadWeapon, bHadPowerfulWeapon;
	local DeusExPlayer dxPlayer;
	...
	// kocmo: random inventory now configured by variable bShifterEnableRandomInventory in DeusEx.ini
	bShifterEnableRandomInventory = true;
	if(DeusExGameInfo(Level.Game) != None)
		bShifterEnableRandomInventory = DeusExGameInfo(Level.Game).bShifterEnableRandomInventory;
	if (!bShifterEnableRandomInventory)
		return true;
	...
	if(bHadWeapon || IsA('HumanMilitary') || IsA('GenericAggressive') || IsA('TNMAggressive') || dxPlayer.combatDifficulty > 4.0)
	{
		...
		{
        		switch(Rand(9)) //Justice: Added grenades
        		{
				case 4:
        			case 0:
        				item = spawn(PS20,self);
				...
				/*case 9: 
					if(bHadWeapon)
					{
						...
						{
							case 0:
								item = spawn(class'WeaponLAM', self);
								break;
							case 1:
								item = spawn(class'WeaponGasGrenade', self);
								break;
							case 2:
								item = spawn(class'WeaponEMPGrenade', self);
								break;
							case 3:
								item = spawn(class'WeaponWPGrenade', self);
						}
					}
					break;*/
        		}
      	...
      	if((item == None && (bHadWeapon == True || IsA('HumanMilitary') || IsA('GenericAggressive') || IsA('TNMAggressive'))) || IsA('MIB') || IsA('WIB') || IsA('TNMMIB') || IsA('TNMWIB') || IsA('WCIDAgent'))
      	{
			...
			//Justice: Added a few items of my own to this
      		switch(Rand(10))
      		{
      			case 0:
      			...
      			case 5:
      			case 6:
					// kocmo: rate-of-fire weapon mod now configured by variable bShifterEnableWeaponModRateOfFire in DeusEx.ini
					bShifterEnableWeaponModRateOfFire = true;
					if(DeusExGameInfo(Level.Game) != None)
						bShifterEnableWeaponModRateOfFire = DeusExGameInfo(Level.Game).bShifterEnableWeaponModRateOfFire;
					if (bShifterEnableWeaponModRateOfFire) {
      			...
      			case 7:
      				...
      				break;
				case 8:
					item = spawn(Class'WeaponPoisonKnives', self);
					break;
				case 9:
					item = spawn(Class'WeaponWPGrenade', self);
					break;
...
{
	if (bShowPain /*&& (TakeHitTimer <= 0)*/) //Justice: NPCs can be stunlocked
		return true;
	else
		return false;
}
                        ...
                        bool bPlayAnim)
{
	...
	if (Health <= 0)
	{
		...
		SkillsForKills(instigatedBy, damageType, HitLocation); //Justice: Skill system disabled by default for BioMod
...
{	
	...
	//Justice: Play the water death sound if appropriate
...
function PlayTakeHitSound(int Damage, name damageType, int Mult)
{
	...
	if (Level.TimeSeconds - LastPainSound < 1.0) //Justice: A longer time between pain sounds to cut down on the "oof oof oof oof oof" factor
		return;
	if (Damage <= 0)
		return;

	LastPainSound = Level.TimeSeconds + fRand(); //Justice: plus some randomness

	//Justice: Use extra pain sounds if appropriate
...
function Frob(Actor Frobber, Inventory frobWith)
{
	...
	if (DeusExPlayer(Frobber) != None)
	{
		if (DeusExPlayer(Frobber).StartConversation(Self, IM_Frob))
		{
			...
			return;
		} //Justice: Pickpocket system.  Putting the "rob" back in frob
		else if(reactionLevel < 1.0 && AICanSee(Frobber, ComputeActorVisibility(Frobber), true, true, true, true) <= 0 && !canConverseWithPlayer(DeusExPlayer(Frobber)))
		{
			item = FindInventoryType(Class'NanoKey');
	
			if (item.IsA('NanoKey'))
			{
				DeusExPlayer(Frobber).PickupNanoKey(NanoKey(item));
				DeusExRootWindow(DeusExPlayer(Frobber).rootWindow).hud.receivedItems.AddItem(item, 1);
				item.Destroy();
				item = None;
			}
		}
...
function Died(pawn Killer, name damageType, vector HitLocation)
{
	...
	if (player != None)
	{
		...
		if(killer == player && !bStunned && bIsHuman && Human(Player) != None) //Justice: Fail the pacifist challenge if the player kills somebody
...
function SkillsForKills(pawn Killer, name damageType, vector HitLocation)
{
	...
	if( (player == DeusExPlayer(Killer) || robokill) && skillon && player != None)
	{
		...
		if(skillon && skillpt != 0) //Now if the level is 0 there's really no point to do any of the rest
		{
			skillpt /= 20; //Get the skill points down to a reasonable level
			if(LastRendered() <= 1.0 || bImportant)
				player.TentativeSkillPointsAdd_ShifterNewSkillSystem(skillpt);
			else //== Silent award
			{
...
state Burning
{
	...
	function Bump(actor Other) //Justice: Fire propagation
	{
		// only take damage every second
		if (Level.TimeSeconds - lastDamageTime >= 1.0)
		{
			//Justice: Don't set the player on fire, because that would be really annoying
			if(Other.IsA('ScriptedPawn') && FRand() > 0.75)
