...
function HackDetected(optional bool bDamageOnly)
{
   ...
   if (Player.Level.NetMode == NM_Standalone)
   {
		if(player.bSpyDroneActive) //Justice: if the player is using a spy drone, shock that instead
			player.aDrone.TakeDamage(200 - 50 * skillLevel, None, vect(0,0,0), vect(0,0,0), 'EMP');
		else
