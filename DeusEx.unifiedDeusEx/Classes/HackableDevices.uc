...
function HackAction(Actor Hacker, bool bHacked)
{
   ...
   else if(Hacker.isA('DeusExPlayer')) //Justice: Autoequip multitools if possible
   {
		multi = DeusExPlayer(Hacker).FindInventoryType(Class'Multitool');
		if(multi != None)
			DeusExPlayer(Hacker).PutInHand(multi);
   }
}
