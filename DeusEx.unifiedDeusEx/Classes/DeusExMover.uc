...
function Frob(Actor Frobber, Inventory frobWith)
{
	...
	local float dotp;
	local DeusExMover M;
	//Justice: vars for autoequipping
	local Inventory unlocker;
	local NanoKeyRing keyring;
	...
	if (!bDone)
	{
		...
		else //Justice: automatically pull out your keyring or lockpicks if appropriate
		{
			unlocker = player.FindInventoryType(Class'Lockpick');

			keyring = NanoKeyRing(player.FindInventoryType(Class'NanoKeyRing'));
			if(keyring != None)
				if(keyring.hasKey(KeyIDNeeded))
					unlocker = keyring;

			if(unlocker != None)
				player.putInHand(unlocker);
		}
	}
...
/*
// kocmo: If infinite recursion is detected, a MessageBox warning pops up, once per map. Player can continue playing after it. To disable, simply comment out this function.
function MakeGroupStopNotify(String notifyMsg)
{
	local bool Mover_InfRecursion_MsgBoxShown;
	local PlayerPawn playerPawn;
	local DeusExPlayer localPlayer;
	local DeusExRootWindow rootWindow;
	local MenuUICustomMessageBoxWindow msgBox;

	Mover_InfRecursion_MsgBoxShown = False;
	playerPawn = GetPlayerPawn();
	if (playerPawn != None)	     localPlayer = DeusExPlayer(playerPawn);
	if (localPlayer != None)     Mover_InfRecursion_MsgBoxShown = localPlayer.flagBase.GetBool('Mover_InfRecursion_MsgBoxShown');

	if (!Mover_InfRecursion_MsgBoxShown) {
		localPlayer.flagBase.SetBool('Mover_InfRecursion_MsgBoxShown', True);
		localPlayer.flagBase.SetExpiration('Mover_InfRecursion_MsgBoxShown', FLAG_Bool, 0);
		rootWindow = DeusExRootWindow(localPlayer.rootWindow);

		msgBox = MenuUICustomMessageBoxWindow(rootWindow.PushWindow(Class'MenuUICustomMessageBoxWindow', False));
		msgBox.SetTitle("Warning");
		msgBox.SetMessageText("Infinite recursion prevented in DeusExMover for "$name$". " $
			"E-mail file 'My Documents\\DeusEx\\System\\Revision.log` to Bjorn <bjorn@dx-revision.com>. Do you want to quit?");
		msgBox.SetMode(0);
