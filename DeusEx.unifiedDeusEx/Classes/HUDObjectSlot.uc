...
event InitWindow()
{
	...
	slotFillWidth = slotFillWidth * dxEnhancedGUIScaleMultiplier;
	slotFillHeight = slotFillHeight * dxEnhancedGUIScaleMultiplier;
	borderHeight = borderHeight * dxEnhancedGUIScaleMultiplier;
	slotIconX = slotIconX * dxEnhancedGUIScaleMultiplier;
	slotIconY = slotIconY * dxEnhancedGUIScaleMultiplier;
	if (dxEnhancedGUIScaleMultiplier > 1) { // nonlinear adjustments by DJ
		borderWidth=84;
		slotNumberX=73;
		slotNumberY=8;
		itemTextPosY_offset = 18;
	}
	...
	itemTextPosY = slotFillHeight - itemTextPosY_offset + slotIconY;
...
event DrawWindow(GC gc)
{
	...
	if ((item != None) && (item.Icon != None) && (!bDragging))
	{
		...
		if (itemText != "")
			gc.DrawText(slotIconX, itemTextPosY, slotFillWidth, itemTextPosY_offset, itemText);
