...
function ScaleDimensions() {
	...
	imageSizeX *= dxEnhancedGUIScaleMultiplier;
	imageSizeY *= dxEnhancedGUIScaleMultiplier;
}
...
event DrawWindow(GC gc)
{
	...
	else
	{
		...
		// don't scale 256x256 textures (relevant for 3rd-party campaigns like Zodiac)
		if (image.imageTextures[0].USize <= 256 && image.imageTextures[0].VSize <= 256) {
		//if (image.imageTextures[0].UClamp <= 256 && image.imageTextures[0].VClamp <= 256) {
