...
function FirstFrame()
{
	...
	local Phone aPhone;
	local bool bShifterEnableWeaponModRateOfFire, bShifterEnableMiniRocketLauncher, bShifterEnableOverpoweredUniqueWeapons;
	...
	if (localURL == "08_NYC_SMUG")
	{
		...
		if(!flags.getBool('M08_Pickup2_Placed'))
		{
			...
			// kocmo: high-powered unique weapons from Shifter now configured by variable bShifterEnableOverpoweredUniqueWeapons in DeusEx.ini
			bShifterEnableOverpoweredUniqueWeapons = true;
			if(DeusExGameInfo(Level.Game) != None)
				bShifterEnableOverpoweredUniqueWeapons = DeusExGameInfo(Level.Game).bShifterEnableOverpoweredUniqueWeapons;
			if (bShifterEnableOverpoweredUniqueWeapons)
			spawn(class'WeaponJackHammer', None,, loc);
			else
				spawn(class'WeaponAssaultShotgun', None,, loc);
	...
	else if(localURL == "08_NYC_UNDERGROUND")
	{
		if(!flags.GetBool('M08_ILAW_Placed'))
		{
			...
			// kocmo: mini rocket launcher now configured by variable bShifterEnableMiniRocketLauncher in DeusEx.ini
			bShifterEnableMiniRocketLauncher = true;
			if(DeusExGameInfo(Level.Game) != None)
				bShifterEnableMiniRocketLauncher = DeusExGameInfo(Level.Game).bShifterEnableMiniRocketLauncher;
			if (bShifterEnableMiniRocketLauncher)
			spawn(class'WeaponMiniRocket', None,, loc);
			else
				spawn(class'WeaponGEPGun', None,, loc);			
	...
	player.dataLinkPlay == None && Level.Netmode == NM_Standalone)
		player.SaveGame(-3, "Auto Save"); //Justice: Autosave after loading a new map... this saves lives!
}
...
defaultproperties
{
     missionName="Hell's Kitchen - Martial Law"
}
