...
{
	local bool bShifterEnabledShotgunAmmoPacking;

	// kocmo: shotgun ammo packing now configured by variable bShifterEnabledShotgunAmmoPacking in DeusEx.ini
	bShifterEnabledShotgunAmmoPacking = true;
	if(DeusExGameInfo(Level.Game) != None)
		bShifterEnabledShotgunAmmoPacking = DeusExGameInfo(Level.Game).bShifterEnabledShotgunAmmoPacking;
	if (!bShifterEnabledShotgunAmmoPacking)
		return;
