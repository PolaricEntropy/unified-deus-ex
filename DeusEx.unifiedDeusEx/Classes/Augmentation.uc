...
//Justice: Modified to work with automatic and passive augs
...
var int alreadyScaledDimensions;
...
//Justice: enum for new aug types
var enum EAugType
{
	active,
	automatic,
...
function ScaleDimensions() {
	...
	if (alreadyScaledDimensions < 1) {
		...
		if (localPlayer != None) {
			dxEnhancedGUIScaleMultiplier = localPlayer.dxEnhancedGUIScaleMultiplier;
			IconWidth *= dxEnhancedGUIScaleMultiplier;
			IconHeight *= dxEnhancedGUIScaleMultiplier;
			alreadyScaledDimensions = 1;
		}
	}
}
...
function Activate()
{
	...
	if (IsInState('Inactive'))
	{
		...
		if (Player.AugmentationSystem.NumAugsActive(true) == 0 && augType == active)
...
function Deactivate()
{
	...
	if (IsInState('Active'))
	{
		...
		if (Player.AugmentationSystem.NumAugsActive(true) == 0)
...
//		else
	...
	if (bIsActive)
	{
		if(augType == passive || augType == automatic) //Justice: If the aug was passive or automatic, reactivate it
			wasOn = true;
		Deactivate();
	}
