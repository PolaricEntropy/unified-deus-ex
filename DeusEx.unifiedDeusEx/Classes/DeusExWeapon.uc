...
function ReloadAmmo()
{
	...
	if (!IsInState('Reload') && CanReload()) //Justice: Prevents you from reloading if you shouldn't be able to
	{
		TweenAnim('Still', 0.1);
		GotoState('Reload');
	}
...
//Justice: Reload function intended for changing to different ammo types
...
//				FireSound=Sound'PlasmaRifleFire';
			...
			//ReloadAmmo();
			ReloadNewAmmo(); //Justice: Use the 'old' function so the reload animation will still play
...
//				PlaySelectiveFiring();
	...
	if (DeusExPlayer(Owner) != None)
	{
		...
		if(!bHandToHand && !IsA('WeaponProd') && !IsA('WeaponMiniCrossbow')) //Justice: Fail the no guns challenge if you fire a gun
			Human(Owner).failChallenge("ChallengeNoGuns");
	}
}
...
simulated function ProcessTraceHit(Actor Other, Vector HitLocation, Vector HitNormal, Vector X, Vector Y, Vector Z)
{
	...
	else if (Other != None)
	{
		...
		else if ((Other != self) && (Other != Owner))
		{
			...
			if((Other.IsA('Button1') || Other.IsA('Switch1') || Other.IsA('Switch2') || Other.IsA('LightSwitch')) && (damageType == 'Shot' || damageType == 'Shell'))
			{
