...
function AddAmmoInfoWindow(DeusExAmmo ammo, bool bShowDescriptions)
{
	...
	if (ammo != None)
	{
		...
		//log("**** PersonaInventoryInfoWindow::AddAmmoInfoWindow(): creating ammo window "$winIcon.width$"x"$winIcon.width$" for ammo icon with size"$temptex.USize$"x"$temptex.VSize$" and clamp "$temptex.UClamp$"x"$temptex.VClamp$"****\n");
