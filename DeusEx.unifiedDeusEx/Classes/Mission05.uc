...
function FirstFrame()
{
	...
	local vector loc;
	local bool bShifterEnableWeaponModRateOfFire;
	...
	if (localURL == "05_NYC_UNATCOMJ12LAB")
	{
		...
		//Justice: Add a WP grenade to the force field locker in the armoury
		if(!flags.GetBool('M05_WPGrenade_Placed'))
		{
			if(spawn(class'WeaponWPGrenade', None,, vect(-8583.000000, 949.000000, 8.000000)) != None)
				flags.SetBool('M05_WPGrenade_Placed',True);
		}
	}
	else if (localURL == "05_NYC_UNATCOHQ")
	{
		...
		foreach AllActors(class'AnnaNavarre', Anna)
		{
			...
			//Justice: Give Anna the key to the front door
			key = spawn(class'NanoKey', Anna);
			if (key != None)
			{
				key.KeyID = 'levelone';
				key.Description = "Level One Entrance";
		...
		//Justice: Add a WP grenade to the hidden section of the supply closet
		if(!flags.GetBool('M05_WPGrenade2_Placed'))
		{
			if(spawn(class'WeaponWPGrenade', None,, vect(-432.000000, 736.000000, 307.000000)) != None)
				flags.SetBool('M05_WPGrenade2_Placed',True);
		}
	}
