...
var float lastFireDamageTime; //Justice: For fire propagation
...
state Burning
{
	...
	//Justice: If we're on fire and we are bumped by a pawn, burn them for their trouble
	function Bump(actor Other)
	{
		if(Other.IsA('Pawn') && !Other.IsA('Robot') && !Region.Zone.bWaterZone && Level.TimeSeconds - lastFireDamageTime >= 1.0)
		{
			...
			lastFireDamageTime = Level.TimeSeconds;
		}
