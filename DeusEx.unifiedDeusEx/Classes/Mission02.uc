...
function FirstFrame()
{
	...
	else if (localURL == "02_NYC_SMUG")
	{
		...
		//Justice: Add an WP grenade behind Smuggler's mirror
		if(!flags.GetBool('M02_WPGrenade_Placed'))
		{
			if(spawn(class'WeaponWPGrenade', None,, vect(-572.000000, 1717.000000, 241.000000)) != None)
				flags.SetBool('M02_WPGrenade_Placed',True);
		}
	}
...
function Timer()
{
	...
	else if (localURL == "02_NYC_SMUG")
	{
		if(!flags.GetBool('M02_ToxinBladeAdded'))
		{
			...
			{
				if(spawn(Class'WeaponCombatKnife', None,, fluffy.Location, rot(0,16383,0) + fluffy.Rotation) != None)
				{
...
defaultproperties
{
     ...
     missionName="Hell's Kitchen - NSF Generator"
}
