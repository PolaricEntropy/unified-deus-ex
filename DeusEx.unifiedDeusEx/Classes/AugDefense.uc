...
//Justice: Modified to be automatic
      ...
      minproj = FindNearestProjectile();
		...
		if (minproj != None)
		{
         ...
         {         
			...
			if (mindist < LevelValues[CurrentLevel] && player.energy > 0)
			{
				minproj.bAggressiveExploded=True;
				minproj.Explode(minproj.Location, vect(0,0,1));
				Player.PlaySound(sound'ProdFire', SLOT_None,,,, 2.0);
				
				if(player.augmentationSystem.getClassLevel(class'AugPower') >= 0.0)
					powerReduce = player.augmentationsystem.getAugLevelValue(class'AugPower');
				else
					powerReduce = 1.0;
					
				player.EnergyDrain += 15 * powerReduce;
				player.EnergyDrainTotal += 15 * powerReduce;
			}
		}
		else
		{
...
defaultproperties
{
     ...
     EnergyRateLabel="Energy Rate: 15 Units/Projectile"
