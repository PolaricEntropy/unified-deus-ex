...
function CreateSnapshotWindow()
{
	...
	winSnapshot.SetPos(467 * dxEnhancedGUIScaleMultiplier - 80, 84 * dxEnhancedGUIScaleMultiplier - 60); // DJ: + 80 and + 60 is to center the smaller image
	//winSnapshot.SetSize(160 * dxEnhancedGUIScaleMultiplier, 120 * dxEnhancedGUIScaleMultiplier); // DJ: Doubling the size doesn't work! kocmo: didn't yet succeed in setting up scaling
...
function UpdateSaveInfo(int rowId)
{
	...
	if (saveDir != None)
	{
		...
		if (saveInfo != None)
		{
			...
			//winSnapshot.SetBackgroundStretching(true);
			//winSnapshot.SetBackgroundSmoothing(true);
