...
simulated function PlayHitSound(actor destActor, Actor hitActor)
{
	...
	if (hitActor != None) 
	{ //Justice: We need to be specific to make sure the sound plays when we want it to
		//if (hitActor.IsA('DeusExDecoration') && (DeusExDecoration(hitActor).minDamageThreshold > 10))
		if (hitActor.IsA('DeusExDecoration'))
		{
			if((DeusExDecoration(hitActor).minDamageThreshold > 10 || DeusExDecoration(hitActor).bInvincible) && !hitActor.IsA('OutdoorThings') && 
			!hitActor.IsA('Roadblock') && !hitActor.IsA('ComputerPersonal') && !hitActor.IsA('InformationDevices') && !hitActor.IsA('Button1'))
			{
				snd = sound'ArmorRicochet';
			}
			else //Justice: Sparks on breakable decorations look weird
			{
				snd = None;
   ...
   if ((!bHandToHand) && bInstantHit && bPenetrating)
	{
      ...
      if (hole != None)      
		...
		if(Other == Level || Other.IsA('DeusExMover') || Other.IsA('DeusExDecoration') || Other.IsA('Robot')) //Justice:  Workaround to get armor ricochets to play
		{
